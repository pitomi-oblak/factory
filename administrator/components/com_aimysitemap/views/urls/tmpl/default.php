 ...
 */
 defined( '_JEXEC' ) or die(); try { JHtml::_( 'jquery.framework' ); JHtml::_( version_compare( JVERSION, '3.3.0', 'lt' ) ? 'behavior.framework' : 'behavior.core' ); JHtml::_( 'formbehavior.chosen', 'select' ); } catch ( Exception $e ) { } require_once( JPATH_COMPONENT . DIRECTORY_SEPARATOR . 'helpers' . DIRECTORY_SEPARATOR . 'rights.php' ); $list_order = $this->escape( $this->state->get( 'list.ordering' ) ); $list_dir = $this->escape( $this->state->get( 'list.direction' ) ); $genericlist_opts = array( 'list.attr' => array( 'class' => 'inputbox' ) ); if ( ! $this->allow_edit ) { $genericlist_opts[ 'list.attr' ][ 'disabled' ] = 'disabled'; } ?>
...
jQuery( document ).ready(function(){
 ...
 echo json_encode( JText::_( 'AIMY_SM_MSG_CONFIRM_RESET_INDEX' ) ); ?> );
    ...
    AimySitemapAjaxInit( '<?php echo JSession::getFormToken(); ?>' );
});
</script>
